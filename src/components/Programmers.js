...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const arr = devList.filter(dev => dev.id === featuredDev);
    return arr[0].name
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredDev !== null ? 'gold': 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          devList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {/* in here set the featured id to be dev.id */setFeaturedDev(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
